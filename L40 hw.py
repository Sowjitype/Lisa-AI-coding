...
def role_based_prompt_activity():
    ...
    student_prompt = f"You are a 5-year-old. Explain {item} in simple terms."
    job_prompt = f"You are an expert in {category} jobs. Explain {item} in detailed, technical terms."

    student_response = generate_response(student_prompt, temperature=0.3, max_tokens=1024)
    job_response = generate_response(job_prompt, temperature=0.3, max_tokens=1024)
    ...
    print("\nReflection:")
    print("1. How did the AI's responses differ between the student and job perspectives?")
    print("2. What are the advantages of using role-based prompts in AI responses for different contexts?")
