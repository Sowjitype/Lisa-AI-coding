...
def get_essay_details():
  ...
  lengths = ["100 words", "400 words", "1200 words", "2000 words"]
  ...
  try:
    idx = int(input(">").strip())
    length = lengths[idx - 1] if 1 <= idx <= len(lengths) else "100 words"
  except ValueError:
    length = "100 words"
...
def generate_essay_content(details):
  ...
  intro = generate_response(intro_p, temperature=temp, max_tokens=1025)
