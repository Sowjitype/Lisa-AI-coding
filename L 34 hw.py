...
recognizer = KaldiRecognizer(model, samplerate, "hello|your name|time|date|exit|quit")
...
def process_query(query):
    ...
    if "time" in query:
        ...
        response = f"The current time is {now}. Let's make the most of our time!"
    elif "date" in query:
        today = datetime.datetime.now().strftime("%B %d, %Y")
        response = f"Today's date is {today}. I hope you have a productive day!"
    else:
