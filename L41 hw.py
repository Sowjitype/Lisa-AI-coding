...
def bias_mitigation_activity():
    ...
    initial_response = generate_response(prompt, temperature=0.5, max_tokens=1024)
    print(f"\nInitial AI Response:\n{initial_response}\n")
...
def token_limit_activity():
    ...
    long_prompt = input("Enter a long prompt (more than 500 words, e.g., 'A detailed story or description'): ").strip()
    
    if long_prompt:
        long_response = generate_response(long_prompt, temperature=0.3, max_tokens=1024)
        preview = (long_response[:1000]+ "...")
